  ...
  for (let count = 0; count < data.length; count++) {
    ...
    switch (type) {
      ...
      case "bulleted_list_item":
        ...
        console.log(
          !data[count + 1] || data[count + 1].type != "bulleted_list_item"
        );
        if (!data[count + 1] || data[count + 1].type != "bulleted_list_item") {
          page += "</ul>\n";
        }
      ...
      case "table":
        ...
        try {
          let tableData = await notion.blocks.children.list({
            block_id: data[count].id,
          });
          ...
          for (let count = 0; count < result.length; count++) {
            ...
            for (
              let itr = 0;
              itr < result[count].table_row.cells.length;
              itr++
            ) {
